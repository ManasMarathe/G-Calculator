...
// params is request data, so it's read in this uncached wrapper; the plain
// jarId string passed down becomes part of the inner cache key.
export default async function Dashboard({ params }: { params: Promise<{ jarId: string }> }) {
  const { jarId } = await params;
  return <DashboardInner jarId={jarId} />;
}

async function DashboardInner({ jarId }: { jarId: string }) {
  "use cache";
  cacheLife("days");
  cacheTag("jar");
  const [jar, { purchases, seshes, sales }] = await Promise.all([
    getJarById(jarId),
    getEverything(jarId),
  ]);
  if (!jar) notFound();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <header className="relative overflow-hidden -mx-4 px-4 py-2">
        ...
        <Link href="/" className="relative text-xs text-muted hover:text-accent transition">
          🫙 switch jar
        </Link>
      </header>
        ...
        {newestFirst.length === 0 ? (
          <div className="rounded-3xl bg-surface border-2 border-edge shadow-sticker p-6 text-center">
            ...
            <p className="text-muted text-sm mt-1">
              head to the{" "}
              <Link href={`/jar/${jarId}/treasury`} className="text-accent underline">treasury</Link> to
              log the first buy
            </p>
          </div>
        ) : (
            ...
            {newestFirst.map((item) => {
              const id =
                item.kind === "purchase"
                  ...
                  : item.kind === "sesh"
                    ? item.sesh.id
                    : item.sale.id;
              // Sesh rows link to their edit page; purchases/sales are inert.
              const row = (
                <>
              ...
              return (
                ...
                >
                  {item.kind === "sesh" ? (
                    <Link
                      href={`/jar/${jarId}/sesh/${item.sesh.id}`}
